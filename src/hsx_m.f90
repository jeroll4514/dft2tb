...
CONTAINS
  ...
  subroutine read_hsx_file_version1(hsx, fname)
    ...
    hsx%has_xij = .true. !I added this afterwards
...
!--------------------------------------------------------------
  ...
  subroutine write_hsx_file_version1(hsx, fname)
    ...
    ! Writes HSX file "fname" 
      ...
      open(hsx_u,file=trim(fname),status='unknown',form='formatted')
    ...
    ! STOP "nnao, no_s..."
      ...
      no_u = hsx%no_u ! Number of Orbitals in Unit cell (no_u)
      no_s = hsx%no_s ! Number of Orbitals in Supercell (no_s)
    
      write(hsx_u,*,iostat=iostat) hsx%gamma ! T or F (Its F for Graphene)
      if (.not. hsx%gamma) then
         write(hsx_u,'(16I3)') (hsx%indxuo(i),i=1,hsx%no_s)
      endif
      ...
      write(hsx_u,'(4I4)',iostat=iostat) (hsx%numh(io), io=1,no_u)      
     ! if (iostat /= 0) STOP "numh"
    
      nnz = sum(hsx%numh(1:hsx%no_u)) ! numh = number of non-zero Hamiltonian elements for a given orbital (nh = numh = 1184)
      if (nnz /= nh) STOP "nnz /= nh"
    
      do io=1,hsx%no_u
         write(hsx_u,'(10I6)',iostat=iostat)  &
                 (hsx%listh(hsx%listhptr(io)+im), im=1,hsx%numh(io)) ! Goes in groups of 8
      enddo
    
      write(hsx_u,*) "HAMILTONIAN SPARSE MATRIX"
      do is=1,hsx%nspin ! only goes through 1 cycle
         do io=1,hsx%no_u ! goes through 8 cycles (8 blocks)
            write(hsx_u,'(10f20.16)',iostat=iostat)   &
                     (real(hsx%hamilt(hsx%listhptr(io)+im,is),kind=dp),   &
                                   im=1,hsx%numh(io)) 
         enddo
      enddo
      !
      !   overlap matrix
      !
      write(hsx_u,*) "OVERLAP SPARSE MATRIX"
      do io=1,hsx%no_u ! goes through 8 cycles (8 blocks)
         write(hsx_u,'(10f20.16)',iostat=iostat)    &
              (real(hsx%Sover(hsx%listhptr(io)+im),kind=dp),im=1,hsx%numh(io))
      enddo
      ...
      !
      do io=1,hsx%no_u ! goes through 8 cycles (8 blocks)
         write(hsx_u,'(3f21.16)',iostat=iostat)    &
                ((real(hsx%xij(k,hsx%listhptr(io)+im),kind=dp), &
                               k=1,3), im=1,hsx%numh(io)) ! 184 elements per block ; groups of 4 coords
      enddo ! 1184 total coordinates
      !
      !        Write auxiliary info
      !
      write(hsx_u,*) "number of species: ", hsx%nspecies !number of differet elements
      nspecies = hsx%nspecies
      print *, "nspecies: ", nspecies
      write(hsx_u,'(A3,1f5.1, 1I2)') (hsx%label(is),hsx%zval(is),hsx%no(is), is=1,nspecies) !label = 4, zval = 0, no = 4
    
      do is=1, nspecies
         do io=1, hsx%no(is) !number of orbitals (4)
            write(hsx_u,'(3I2)') hsx%nquant(is,io), hsx%lquant(is,io), hsx%zeta(is,io)
         enddo
      enddo
      write(hsx_u,*) "number of atoms in uc: ", hsx%na_u !number of atoms in unit cell
    
      na_u = hsx%na_u
    
      write(hsx_u,'(I2)') (hsx%isa(ia), ia=1,na_u)
      write(hsx_u,'(2I2)') (hsx%iaorb(io), hsx%iphorb(io), io=1,no_u)
    
      write(hsx_u,*) (hsx%listhptr(io),io=1,hsx%no_u)
